...
const projectContentDir = './src/content/projects';
const projectTemplateDir = './templates/project.mdx';
...
async function main() {
  ...
  const template = await readFile(projectTemplateDir, 'utf8');
  ...
  const content = template
    ...
    .replaceAll('{{site}}', site || 'null')
    .replaceAll('{{repository}}', repository || 'null');

  const contentPath = path.join(projectContentDir, `${slug}.mdx`);
